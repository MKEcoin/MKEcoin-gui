; MKEcoin Oxygen Orion GUI Wallet Installer for Windows
; Copyright (c) 2017-2020, The MKEcoin Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\MKEcoin-wallet-gui.exe")

[Setup]
AppName=MKEcoin GUI Wallet
...
DefaultDirName={commonpf}\MKEcoin GUI Wallet
DefaultGroupName=MKEcoin GUI Wallet
UninstallDisplayIcon={app}\MKEcoin-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The MKEcoin Developer Community
AppPublisherURL=https://getMKEcoin.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the MKEcoin files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to MKEcoin.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; MKEcoin GUI wallet exe and guide
Source: "bin\MKEcoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\MKEcoin-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; MKEcoin CLI wallet
Source: "bin\extras\MKEcoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; MKEcoin wallet RPC interface implementation
Source: "bin\extras\MKEcoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; MKEcoin daemon
Source: "bin\MKEcoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; MKEcoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "MKEcoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; MKEcoin blockchain utilities
Source: "bin\extras\MKEcoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\MKEcoin-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\MKEcoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitMKEcoin');
  s := 'The default folder to store the MKEcoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\MKEcoin-project\MKEcoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" MKEcoin default location
...
begin
  Result := BlockChainDir('') + '\bitMKEcoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "MKEcoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the MKEcoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\MKEcoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\MKEcoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "MKEcoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\MKEcoin-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\MKEcoin-gui-wallet-guide.pdf"; IconFilename: "{app}\MKEcoin-wallet-gui.exe"
...
Name: "{group}\Utilities\MKEcoin Daemon"; Filename: "{app}\MKEcoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\MKEcoin-wallet-cli.exe"; WorkingDir: "{userdocs}\MKEcoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\MKEcoin\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\MKEcoin-wallet-gui\MKEcoin-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\MKEcoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM MKEcoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\MKEcoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\MKEcoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\MKEcoin-project\MKEcoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\MKEcoin-project\MKEcoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "MKEcoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "MKEcoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "MKEcoin"; ValueType: "string"; ValueData: "URL:MKEcoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "MKEcoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "MKEcoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\MKEcoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "MKEcoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\MKEcoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "MKEcoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "MKEcoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "MKEcoinseed"; ValueType: "string"; ValueData: "URL:MKEcoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "MKEcoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "MKEcoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\MKEcoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "MKEcoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\MKEcoin-wallet-gui.exe"" ""%1"""
